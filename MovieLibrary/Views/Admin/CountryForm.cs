...
namespace MovieLibrary.Views.Admin
{
    public partial class CountryForm : Form
    {
        ...
        public CountryForm(Table table)
        {
            _table = table;
        ...
        private async void btnUpsert_Click(object sender, EventArgs e)
        {
            ...
            var model = (Country)_manager.modelProvider.GetModel(index, grid, _table.TableName);
            if (model.Id == null)
            {
                if (_manager.countryTable.Add(model))
                {
            ...
            {
                if (_manager.countryTable.Update(model))
                {
        ...
        private async void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            var countryManager = _manager.countryTable;
            if (countryManager.Delete(index))
            {
        ...
        private async void btnFind_Click_1(object sender, EventArgs e)
        {
            ...
            var allCountries = await _manager.countryTable.GetAll();
            if (allCountries.Count > 0)
            {
                    ...
                    if (int.TryParse(enteredValue, out id))
                    {
                        ...
                        if (country != null)
                        {
                            ...
                            if (grid.Rows.Count > 0)
                            {
                                ...
                                {
                                    if (row.Cells["id"].Value.ToString() == country.Id.ToString())
                                    {
                ...
                {
                    var listOfCountries = allCountries.Where(x => x.Name.ToLower().Contains(enteredValue.ToLower())).ToList();
                    ...
                    if (listOfCountries.Count > 0)
                    {
                        ...
                        {
                            grid.Rows.Add(new object[] { country.Id, country.Name });
                        }
